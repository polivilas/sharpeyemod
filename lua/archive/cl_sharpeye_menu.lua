...
function sharpeye.BuildMenu( opt_tExpand )
	...
	*/
	...
	do
		...
		GeneralBreathingMulti.OnSelect = function(index, value, data)
			if value == 2 then
				sharpeye:SetVar( "detail_focus_aimsim" , 5 )
				sharpeye:SetVar( "detail_focus_handshiftx" , 0 )
				
			elseif value == 3 then
				sharpeye:SetVar( "detail_focus_aimsim" , 8 )
				sharpeye:SetVar( "detail_focus_handshiftx" , 4 )
		...
		do
			local as = sharpeye:GetVar("detail_focus_aimsim")
			local hs = sharpeye:GetVar("detail_focus_handshiftx")
	...
	sharpeye.Util_AppendLabel( refPanel, "Wiimote support only affects aiming. Please use GlovePIE or similar to configure button input.", 50, true )
